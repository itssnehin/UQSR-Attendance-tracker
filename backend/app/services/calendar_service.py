...
class CalendarService:
    ...
    def configure_run_day(self, target_date: date, has_run: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            existing_config = (
                self.db.query(CalendarConfig)
                .filter(CalendarConfig.date == target_date)
                .first()
            )
            ...
            if existing_config:
                ...
                logger.info(
                    f"Updated calendar config for {target_date}: has_run={has_run}"
                )
            else:
                # Create new configuration
                new_config = CalendarConfig(date=target_date, has_run=has_run)
                self.db.add(new_config)
                logger.info(
                    f"Created calendar config for {target_date}: has_run={has_run}"
                )
        ...
        try:
            ...
            for config in configs:
                ...
                if config.has_run:
                    run = (
                        self.db.query(Run)
                        .filter(Run.date == config.date, Run.is_active == True)
                        .first()
                    )
                    ...
                    if run:
                        ...
                        attendance_count = (
                            ...
                            .filter(Attendance.run_id == run.id)
                            .count()
                        )
    ...
    def get_today_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            config = (
                self.db.query(CalendarConfig)
                .filter(CalendarConfig.date == today)
                .first()
            )
            ...
            run = (
                self.db.query(Run)
                .filter(Run.date == today, Run.is_active == True)
                .first()
            )
            ...
            if not run:
                ...
                logger.warning(
                    f"Calendar config indicates run for {today} but no Run record found"
                )
                return {
                    "success": True,
                    "has_run_today": True,
            ...
            attendance_count = (
                self.db.query(Attendance).filter(Attendance.run_id == run.id).count()
            )

            logger.info(
                f"Today's run status: session_id={run.session_id}, attendance={attendance_count}"
            )
    ...
    def _ensure_run_exists(self, target_date: date) -> None:
        ...
        """
        existing_run = self.db.query(Run).filter(Run.date == target_date).first()

        if not existing_run:
            # Generate unique session ID
            session_id = self._generate_session_id(target_date)

            new_run = Run(date=target_date, session_id=session_id, is_active=True)
            self.db.add(new_run)
            logger.info(
                f"Created new run for {target_date} with session_id={session_id}"
            )
    ...
    def _generate_session_id(self, target_date: date) -> str:
        """
        Generate a simple 5-digit attendance code for a run.

        Args:
            target_date: Date for the run

        Returns:
            5-digit attendance code
        """
        import random
    ...
    def get_run_by_session_id(self, session_id: str) -> Optional[Run]:
        ...
        try:
            return (
                self.db.query(Run)
                .filter(Run.session_id == session_id, Run.is_active == True)
                .first()
            )
        except Exception as e:
